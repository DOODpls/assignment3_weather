      ...
      $("#alerts").click(
        function() {
          $('#today-container').append('<div class="alertinfo" id="alertinfo"></div>');
            $('#alertinfo').toggle( function(){
              document.querySelector('#alertinfo').innerHTML = '<p class="from-to">' + from + "to" + to + '</p>';
              document.querySelector('#alertinfo').innerHTML = '<p class="alert-description">' + description + '</p>';
            })
        });
    }else{
      $('#alerts').remove();
