...
function search(ele) {
  if(event.key === 'Enter') {
    var url3 = 'api.opencagedata.com/geocode/v1/json?key=3eb2cc6ab0ac4ef986e4c30c2da5e38c&q=' + ele.value + '&pretty=1&no_annotations=1'   //this is where i convert the long,lat to city name, this api has a limit, so please dont abuse it huhuhuhuu
    fetch(url3)
    .then(z => z.json())
    .then(z => {
      if(ele.value == ''){
        ...
        locres = (z.result.geometry.lat+','+z.geometry.lng)
      render()
      resetDaily()
      locationname()
      }
