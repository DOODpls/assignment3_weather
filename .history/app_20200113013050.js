...
$('#alerts').click(function() {
  alert(title + description + from + to)
}) ;
...
function render(){
  ...
  try{
    ...
    from = new Date( x.alerts[0].time *1000).toLocaleTimeString("en-US", {timeZone: x.timezone}).replace(/([\d]+:[\d]{2})(:[\d]{2})(.*)/, "$1$3");
    to = new Date( x.alerts[0].expires *1000).toLocaleTimeString("en-US", {timeZone: x.timezone}).replace(/([\d]+:[\d]{2})(:[\d]{2})(.*)/, "$1$3");
  }catch{
